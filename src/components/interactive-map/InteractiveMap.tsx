...
const p21Catalogs=[{label:"CATALOGO JOPACK",url:"https://lojaredemaxxi.com.br/marcas/jopack-industrial-eireli"},{label:"CATALOGO UNIFORTE",url:"https://lojaredemaxxi.com.br/marcas/unifortte-industria-de-plasticos-ltda"}];
 ...
 return <main ref={appShell} className="app-shell">
  ...
  <section className="workspace">
   ...
   <aside className="side-panel">
    <div className="search-block" onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setSearchOpen(false)}}><label htmlFor="exhibitor-search">BUSCAR EXPOSITOR</label><div className="search-input"><Search/><input id="exhibitor-search" value={query} onFocus={()=>{setKeyboardOpen(true);if(!query&&!searchOpen)refreshSuggestions()}} onClick={()=>{setKeyboardOpen(true);if(!query)refreshSuggestions()}} onChange={e=>{setQuery(e.target.value);setSearchOpen(true);if(e.target.value)track("exhibitor_search",{query:e.target.value})}} placeholder="Marca, categoria ou stand" autoComplete="off" role="combobox" aria-autocomplete="list" aria-expanded={searchOpen} aria-controls="exhibitor-results"/>{query&&<button onClick={()=>{setQuery("");refreshSuggestions()}} aria-label="Limpar busca"><X/></button>}</div>
  ...
  {keyboardOpen&&<div className="virtual-keyboard" role="group" aria-label="Teclado para busca" onPointerDown={event=>event.preventDefault()}><div className="keyboard-top"><strong>DIGITE O EXPOSITOR, MARCA OU STAND</strong><button onClick={()=>setKeyboardOpen(false)} aria-label="Fechar teclado">FECHAR <X/></button></div>{keyboardRows.map((row,index)=><div className="keyboard-row" key={index}>{row.map(key=><button key={key} onClick={()=>typeOnKeyboard(key)}>{key}</button>)}</div>)}<div className="keyboard-row keyboard-actions"><button onClick={()=>setQuery("")} className="keyboard-clear">LIMPAR</button><button onClick={()=>typeOnKeyboard("SPACE")} className="keyboard-space" aria-label="Espaço">ESPAÇO</button><button onClick={()=>typeOnKeyboard("BACKSPACE")} aria-label="Apagar último caractere">⌫ APAGAR</button><button onClick={()=>setKeyboardOpen(false)} className="keyboard-done">CONCLUIR</button></div></div>}
