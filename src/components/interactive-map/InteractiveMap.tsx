...
const standCatalogs:Record<string,{label:string;url:string}[]>={
 P21:[{label:"CATÁLOGO JOPACK",url:"https://lojaredemaxxi.com.br/marcas/jopack-industrial-eireli"},{label:"CATÁLOGO UNIFORTE",url:"https://lojaredemaxxi.com.br/marcas/unifortte-industria-de-plasticos-ltda"}],
 P23:[{label:"CATÁLOGO FC",url:"https://lojaredemaxxi.com.br/marcas/f-c-ind-e-com-de-comp-eletricos-e-eletronicos-ltda"},{label:"CATÁLOGO ALIANÇA",url:"https://lojaredemaxxi.com.br/marcas/alianca-metalurgica-sa"}],
};
const keyboardRows=[["1","2","3","4","5","6","7","8","9","0"],["Q","W","E","R","T","Y","U","I","O","P"],["A","S","D","F","G","H","J","K","L","Ç"],["Z","X","C","V","B","N","M"]];
...
function ExhibitorLogo({exhibitor}:{exhibitor:Exhibitor}){
 ...
 return <div className="result-logo" aria-hidden="true"><svg viewBox={`${cx-side/2} ${cy-side/2} ${side} ${side}`} preserveAspectRatio="xMidYMid meet"><g transform={`rotate(${-angle} ${cx} ${cy})`}><image href="/reference/fenarem-reference.svg" x="0" y="0" width="8000" height="4500"/></g></svg></div>;
}
...
export function InteractiveMap(){
 ...
 const [catalog,setCatalog]=useState<{name:string;url:string}|null>(null); const [focus,setFocus]=useState<Focus>(initialFocus); const [warning,setWarning]=useState(false);
 const [keyboardOpen,setKeyboardOpen]=useState(false); const [fullscreen,setFullscreen]=useState(false); const [dragging,setDragging]=useState(false); const [draft,setDraft]=useState<{start:Point;end:Point}|null>(null); const appShell=useRef<HTMLElement>(null); const viewport=useRef<HTMLDivElement>(null); const drag=useRef<{clientX:number;clientY:number;focus:Focus}|null>(null); const dragMoved=useRef(false); const moveFrame=useRef<number|null>(null); const pendingFocus=useRef<Focus|null>(null);
 ...
 const typeOnKeyboard=(key:string)=>{setQuery(current=>key==="BACKSPACE"?current.slice(0,-1):key==="SPACE"?current+" ":current+key);setSearchOpen(true)};
 const reset=useCallback(()=>{setQuery("");setSearchOpen(false);setKeyboardOpen(false);setSelected(null);setPlace(null);setCatalog(null);setFocus(initialFocus);setWarning(false);track("map_reset");},[]);
 ...
 const chooseExhibitor=(e:Exhibitor,source:"result"|"stand")=>{const geometry=[...stands,...specialAreas].find(area=>(area.companyId??area.id)===e.id||area.code===e.stand);const selectedExhibitor=geometry?{...e,hotspot:areaHotspot(geometry)}:e;setSelected(selectedExhibitor);setPlace(null);setQuery("");setSearchOpen(false);setKeyboardOpen(false);track(source==="result"?"exhibitor_result_clicked":"exhibitor_stand_clicked",{id:e.id});};
 const chooseLocation=(l:MapLocation)=>{setPlace(l);setSelected(null);track("location_clicked",{id:l.id});};
 ...
 return <main ref={appShell} className="app-shell">
  <header className="topbar"><Image className="brand-logo" src="/fenarem-logo.png" alt="FENAREM — Feira de Negócios Anual da Rede Elétrica e Maxxirede — 2ª Edição" width={1396} height={417} priority /><div className="top-title"><span>MAPA INTERATIVO</span><small>Encontre marcas, stands e serviços</small></div><button className="home-button" onClick={reset}><Home/> CENTRALIZAR MAPA</button></header>
  <section className="workspace">
   ...
   <aside className="side-panel">
    <div className="search-block" onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget as Node))setSearchOpen(false)}}><label htmlFor="exhibitor-search">BUSCAR EXPOSITOR</label><div className="search-input"><Search/><input id="exhibitor-search" value={query} onFocus={()=>{setKeyboardOpen(true);if(!query&&!searchOpen)refreshSuggestions()}} onClick={()=>{setKeyboardOpen(true);if(!query)refreshSuggestions()}} onChange={e=>{setQuery(e.target.value);setSearchOpen(true);if(e.target.value)track("exhibitor_search",{query:e.target.value})}} onKeyDown={event=>{if(event.key==="Enter"&&query&&results[0]){event.preventDefault();chooseExhibitor(results[0],"result")}}} placeholder="Marca, categoria ou stand" autoComplete="off" role="combobox" aria-autocomplete="list" aria-expanded={searchOpen} aria-controls="exhibitor-results"/>{query&&<button onClick={()=>{setQuery("");refreshSuggestions()}} aria-label="Limpar busca"><X/></button>}</div>
  ...
  {(selected||place)&&<div className={`details-card${selected?" exhibitor-card":place?.kind!=="stand"?" location-card":""}`} role="dialog" aria-modal="false" aria-label={selected?`Informações de ${selected.name}`:`Informações de ${place!.name}`}><button className="close" onClick={()=>{setSelected(null);setPlace(null)}} aria-label="Fechar detalhes" title="Fechar"><X/></button>{selected?<><div className="panel-heading"><span>STAND {selected.stand}</span><h2>{selected.name}</h2>{selected.category&&<p>{selected.category}</p>}</div><div className="stand-code"><span>LOCALIZAÇÃO NO EVENTO</span><strong>STAND {selected.stand}</strong></div><div className="panel-actions">{standCatalogs[selected.stand]?standCatalogs[selected.stand].map(option=><button key={option.url} className="primary" onClick={()=>{setCatalog({name:option.label,url:option.url});track("catalog_opened",{id:selected.id,catalog:option.label})}}>{option.label}</button>):selected.catalogUrl?<button className="primary" onClick={()=>{setCatalog({name:selected.name,url:selected.catalogUrl!});track("catalog_opened",{id:selected.id})}}>ACESSAR CATÁLOGO</button>:<button className="primary" disabled>CATÁLOGO INDISPONÍVEL</button>}</div></>:<><span className="eyebrow">LOCALIZAÇÃO</span><div className="detail-icon"><MapPin/></div><h2>{place!.name}</h2>{place!.kind==="stand"&&<b className="stand-label">STAND {place!.shortName}</b>}<p>{place!.kind==="stand"?"Stand participante da FENAREM.":"Área localizada neste ponto da feira."}</p>{place!.kind==="stand"&&<><small>Os dados comerciais e o catálogo deste expositor ainda não foram cadastrados.</small><button className="primary" onClick={()=>center(place!.hotspot)}>CENTRALIZAR NO MAPA</button></>}</>}<button className="back-link" onClick={()=>{setSelected(null);setPlace(null);setFocus(initialFocus)}}><ArrowLeft/> Voltar ao mapa</button></div>}
  {catalog&&<div className="catalog-backdrop" role="dialog" aria-modal="true" aria-label={`Catálogo ${catalog.name}`} onMouseDown={(event)=>{if(event.target===event.currentTarget)setCatalog(null)}}><div className="catalog-window"><header><div><span>CATÁLOGO</span><strong>{catalog.name}</strong></div><button onClick={()=>setCatalog(null)} aria-label="Fechar catálogo"><X/></button></header><iframe src={catalog.url} title={`Catálogo ${catalog.name}`} loading="eager" referrerPolicy="no-referrer-when-downgrade"/></div></div>}
  {keyboardOpen&&<div className="virtual-keyboard" role="group" aria-label="Teclado para busca" onPointerDown={event=>event.preventDefault()}><div className="keyboard-top"><strong>DIGITE O EXPOSITOR, MARCA OU STAND</strong><button onClick={()=>{setKeyboardOpen(false);setSearchOpen(false)}} aria-label="Fechar teclado e resultados da busca">FECHAR <X/></button></div>{keyboardRows.map((row,index)=><div className="keyboard-row" key={index}>{row.map(key=><button key={key} onClick={()=>typeOnKeyboard(key)}>{key}</button>)}</div>)}<div className="keyboard-row keyboard-actions"><button onClick={()=>setQuery("")} className="keyboard-clear">LIMPAR</button><button onClick={()=>typeOnKeyboard("SPACE")} className="keyboard-space" aria-label="Espaço">ESPAÇO</button><button onClick={()=>typeOnKeyboard("BACKSPACE")} aria-label="Apagar último caractere">⌫ APAGAR</button><button onClick={()=>{if(query&&results[0]){chooseExhibitor(results[0],"result");return}setKeyboardOpen(false)}} className="keyboard-done">CONCLUIR</button></div></div>}
